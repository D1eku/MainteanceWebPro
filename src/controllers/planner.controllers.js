...
const sendAssignDatePauta = async(req, res) => {
    try {
        ...
        if (codExiste < 1) {
            ...
            const insertCalendario = await pool.query('insert into calendario values($1, $2, $3, $4, $5, $6)', [codigo_calendario, pautaActual.equipo, rut_mantenedor, fEstimada, codigo_pauta, req.session.rut]);
            res.redirect("/pln")
        } else {
